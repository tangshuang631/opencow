...
Latest same-host native baseline (one sampled run; not a release threshold): cold total `1,451.90ms` / load `7.66ms` / prompt `14.18 tok/s` / decode `20.83 tok/s`; warm total `134.85ms` / load `5.51ms` / decode `16.48 tok/s`; cache continuation candidate ratio `0.350` (`302.41ms → 105.81ms`, stable prefix `115` chars); embedding `3,087.66ms`, dimension `4,096`. Cold and warm values are kept separate and all measurements are machine-state dependent.
...
- The gate is now enforced inside `runFastLane`; default flags remain off. The bounded `PerformanceProfileStore`, explicit native cold/warm benchmark, and `/api/ps` residency mapping are implemented and tested, while production diagnostic wiring/sample scheduling, the required 8K-token cache continuation benchmark, and WP1C formal evaluation/native-desktop/migration evidence remain open.
