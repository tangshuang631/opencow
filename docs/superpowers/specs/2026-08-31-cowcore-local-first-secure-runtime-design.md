...
当前 `dev` 工作阶段（2026-09-04）已加入但尚未作为 WP1 出口冻结的核心切片：`@opencow/cowcore` 的 Ollama Native API/Profile/Locality/Residency（含 `/api/ps` 只读映射）/Context/Prefix/keep-alive、确定性 TaskClass、零效果 typed-tool loop、Fast Lane、版本化 PerformanceProfileStore，以及显式固定 fixture 的冷/温 benchmark；桌面 native profile smoke wrapper 已可携带 ProfileStore。桌面交互已增加流式 chunk 的帧级合并与完成/取消 flush，但没有做新的视觉换肤。新增能力仍不替换旧 Ollama 业务链路，也没有新增宿主执行能力。WP1A/WP1B 出口仍需完成生产本地运行时生命周期接线、桌面诊断调度与实际 Profile/Probe/metrics 采集、8K cache continuation 真实性能验证和 flag/回滚验证后才能标记完成。
