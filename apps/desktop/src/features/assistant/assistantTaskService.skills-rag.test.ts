...
describe("assistantTaskService skill-assisted readonly RAG execution", () => {
  ...
  it("plans a Chinese skill-assisted readonly local RAG search without a permission upgrade", () => {
    const plan = planAssistantTask(
      "用已启用 docs skill 搜索本地规则里的 shell permission guidance",
      "readonly"
    );
  ...
  it("executes a Chinese skill-assisted local RAG search with productized Chinese output", async () => {
    matchEnabledLocalSkillsMock.mockResolvedValueOnce({
      query: "用已启用 docs skill 搜索本地规则里的 shell permission guidance",
      summary: "Enabled local skill matching found 1 recommended skill across 2 enabled entries.",
      registry_path: ".opencow/skills/enabled-skills.json",
      enabled_skill_count: 2,
      match_count: 1,
    ...
    const result = await executeAssistantTask({
      kind: "skills-local-enabled-rag-doc-search",
      title: "Skill-assisted local RAG document search",
      summary: "用已启用 docs skill 搜索本地规则里的 shell permission guidance",
      auditSummary: "Local assistant planned a skill-assisted readonly local RAG document search.",
      auditDetail: "Skill-assisted readonly local RAG search task"
    } as const);

    expect(matchEnabledLocalSkillsMock).toHaveBeenCalledWith(
      "用已启用 docs skill 搜索本地规则里的 shell permission guidance"
    );
    ...
    expect(result.resultSummary).toContain("主要来源：04-permission-safety-shell.md、OPENCOW_CORE_RULES.md。");
    expect(result.resultSummary).toContain("检索问题：用已启用 docs skill 搜索本地规则里的 shell permission guidance。");
